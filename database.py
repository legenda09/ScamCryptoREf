...
async def adding_data(user_id, user_username, referrer, referrer_bonus, entry_date):
    async with aiosqlite.connect("database.db") as db:
        await db.execute("INSERT INTO database (user_id, user_username, user_balance, referrer_id, referrer_bonus, entry_date) VALUES (?, ?, ?, ?, ?, ?)",
...
async def select_entry_date_where_user_id(user_id):
    async with aiosqlite.connect("database.db") as db:
        async with db.execute("SELECT entry_date FROM database WHERE user_id = ?", (user_id,)) as cursor:
...
async def select_user_id_where_referrer_id_and_referrer_bonus_1(user_id):
    async with aiosqlite.connect("database.db") as db:
        async with db.execute("SELECT user_id FROM database WHERE referrer_id = ? AND referrer_bonus = 1", (user_id,)) as cursor:
...
async def changing_user_balance_where_user_username(change_amount, username_or_id):
    async with aiosqlite.connect("database.db") as db:
        await db.execute("UPDATE database SET user_balance = ? WHERE user_username = ?", (change_amount, username_or_id))
        await db.commit()


async def adding_user_balance_where_user_id(referral):
    async with aiosqlite.connect("database.db") as db:
        await db.execute("UPDATE database SET user_balance = user_balance + ? WHERE user_id = ?", (amount_per_one, referral,))
        await db.commit()
